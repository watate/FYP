#This is generally the same as DDPG.py but it trains it without the jerk penalty, and trains with velocity smoother
...
from StageWorld_VSmooth import StageWorld
...
velocity_list_filename = 'vsmooth_complex_velocity_list.dat'
replay_buffer_filename = 'replay_buffer.dat'
current_date_time = time.strftime("%Y%m%d-%H%M%S")

config_notes = """Collecting velocity data for Vsmooth Complex Model""" #Write stuff here to explain what training was for
# ===========================
...
def train(sess, env, actor, critic, noise, reward, discrete, action_bound):
    ...
    for i in range(MAX_EPISODES):
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            #Linear Velocity Smoother
            if target_speed[0] > control_speed[0]:
                control_speed[0] = min(target_speed[0], control_speed[0] + 0.10)
            elif target_speed[0] < control_speed[0]:
                control_speed[0] = max(target_speed[0], control_speed[0] - 0.10)
            else:
                control_speed[0] = target_speed[0]

            #Angular Velocity Smoother
            if target_speed[1] > control_speed[1]:
                control_speed[1] = min(target_speed[1], control_speed[1] + 0.20)
            elif target_speed[1] < control_speed[1]:
                control_speed[1] = max(target_speed[1], control_speed[1] - 0.20)
            else:
                control_speed[1] = target_speed[1]
            #############################################
            env.Control(control_speed)
            ###################################################################################
            #Append to velocity list
            if save_velocity_bool == 1:
                velocity_list.append((i, control_speed[0], control_speed[1]))
            ###################################################################################
...
def main(_):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        state_dim = LASER_BEAM * LASER_HIST + TARGET + SPEED
