...
class StageWorld():
	def __init__(self, beam_num, map_type):
		...
		##################### NEW PARAMS ###################################
		self.self_accel = [0.0, 0.0]
		self.self_jerk = [0.0, 0.0]
		#self.previous_jerk_reward = 0
	...
	def ResetWorld(self):
		self.ResetStage()
		self.self_speed = [0.0, 0.0]
		self.self_accel = [0.0, 0.0] #new
		self.self_jerk = [0.0, 0.0] #new
		self.step_target = [0., 0.]
		self.step_r_cnt = 0.
		self.start_time = time.time()
		rospy.sleep(0.5)
	...
	def UpdateAccelAndJerk(self, accel, jerk):
		self.self_accel = accel
		self.self_jerk = jerk

	def GetRewardAndTerminate(self, t, jerk):
		terminate = False
		reset = False
		...
		reward = (self.pre_distance - self.distance) * np.cos(w) - 0.005#0.01

		#I'm adding the discount factor and recurrence of past rewards
		#reward = reward + (0.90)*self.previous_reward #0.5 is discount factor
		#self.previous_reward = reward

		# if robot doesn't move, penalize it with 0.01
		#if (self.pre_distance - self.distance == 0):
		#	reward = reward - 0.2
		
		#############Add New Reward Function########################
		#we can't exactly reward jerk reduction (because what if robot generates jerk just to reduce it to get reward?)
		#we need to penalize bad behaviour: jerk
		#theoretical maximum jerk penalty is -2
			#we need to scale that to the size of the reward received from getting closer to the target
			#I divide the jerk generated by 100 so the max jerk is 0.04
		jerk_reward = ((jerk[0]/1)**2 + (jerk[1]/(4*np.pi/3)**2))/50
		#reward = reward - jerk_reward
		#self.previous_jerk_reward = jerk_reward
		############################################################
		# reward = -0.5 * 0.2
		result = 0
